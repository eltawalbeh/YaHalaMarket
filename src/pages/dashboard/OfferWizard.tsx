...
export default function OfferWizard() {
  ...
  const createDraft = async () => {
    ...
    await offersService.create({
      ...
      duration_nights: Number(form.nights) || 0,
      departure_dates: [],
      hotel_ids: [],
        ...
        <div className="flex items-center gap-2 mb-8 overflow-x-auto">
          {STEPS.map((item, itemIndex) => (
            <div key={item} className="flex items-center gap-2 shrink-0">
              <span
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold numerals-latin ${
                  itemIndex <= index
              ...
              <span className="text-sm">
                {lang === "ar" ? LABELS[item].ar : LABELS[item].en}
              </span>
          ...
          {step === "basics" && (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field
                ...
                onChange={(v) => update("country_ar", v)}
              />
              <Field
                label={lang === "ar" ? "عدد الليالي" : "Nights"}
                type="number"
                value={form.nights}
                onChange={(v) => update("nights", v)}
              />
              <Field
                label={lang === "ar" ? "تاريخ انتهاء العرض" : "Offer expiry"}
                type="date"
          ...
          {step === "details" && (
            <div className="grid gap-4">
              ...
              <p className="text-sm text-[var(--muted-foreground)]">
                {lang === "ar"
                  ? "سيتم ربط الفنادق والصور والمستندات في المرحلة التالية."
                  : "Hotels, images and documents will be connected in the next phase."}
              </p>
            </div>
          )}
          {step === "review" && (
            <div className="space-y-3 text-sm">
              <p>
                <strong>{lang === "ar" ? "العنوان" : "Title"}:</strong>{" "}
                {form.title_ar || form.title}
              </p>
              <p>
                <strong>{lang === "ar" ? "الوجهة" : "Destination"}:</strong>{" "}
                {form.city_ar || form.city}, {form.country_ar || form.country}
              </p>
              <p>
              ...
              <p className="text-[var(--muted-foreground)]">
                {lang === "ar"
                  ? "سيتم حفظ العرض كمسودة."
                  : "The offer will be saved as a draft."}
              </p>
            </div>
          )}
        </Card>
...
function Field({
  ...
  const className =
    "w-full px-3 py-2 text-sm border border-[var(--border)] rounded-[var(--radius)] bg-[var(--background)] focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
  return (
