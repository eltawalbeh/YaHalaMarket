...
export default function Offers() {
  ...
  useEffect(() => { offersService.list().then(setOffers); }, []);
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {offers.map((offer) => {
              ...
              return (
                <tr key={offer.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--muted)] transition-colors">
                  <td className="px-4 py-3 font-medium">{title}</td>
                  <td className="px-4 py-3 text-[var(--muted-foreground)] hidden sm:table-cell">{city}</td>
                  <td className="px-4 py-3">
                    <Badge variant={statusMeta.color as 'green' | 'gray' | 'yellow' | 'red'}>
