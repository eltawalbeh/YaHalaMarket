...
export default function Hotels() {
  ...
  useEffect(() => { hotelsService.list().then(setHotels); }, []);
    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {hotels.map((hotel) => {
          ...
          const city = lang === 'ar' ? hotel.destination_city_ar : hotel.destination_city;
          return (
            <Card key={hotel.id}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-semibold">{name}</p>
                  <p className="text-sm text-[var(--muted-foreground)]">{city} · {hotel.destination_country_code}</p>
