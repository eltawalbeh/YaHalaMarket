    ...
    <PublicLayout>
      <article className="max-w-3xl mx-auto">
        <Link
          ...
          className="text-sm text-[var(--muted-foreground)] hover:text-[var(--primary)] mb-6 inline-block"
        >
          ← {t("offers", lang)}
        </Link>
        <div
          className="h-64 rounded-[var(--radius)] bg-[var(--muted)] flex items-center justify-center mb-6"
          role="img"
          aria-label={title}
        >
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          ...
          <DetailBlock
            ...
            value={
              lang === "ar"
                ? "خيارات الفنادق تظهر في النسخة المتصلة بالبيانات"
                : "Hotel options will be supplied by the connected data layer"
            }
          />
        </section>
      </article>
