...
export const quotesService: QuotesService = {
  async list(filters) {
    if (!supabase) {
      ...
      if (filters?.status)
        results = results.filter((quote) => quote.status === filters.status)
      if (filters?.lead_id)
        results = results.filter((quote) => quote.lead_id === filters.lead_id)
      return results
    }
    let query = supabase
      .from("quotes")
  ...
  async getByToken(token) {
    if (!supabase)
      return mockQuotes.find((quote) => quote.token === token) ?? null
    const result = await supabase
      .from("quotes")
      .select("*")
      .eq("token", token)
